...
release_dir="release/zenodo-v1.16-submission-framing-repair"
zip_path="${release_dir}.zip"
pdf_name="demand-forecasting-submission-framing-repair-v1.16.pdf"
...
python3 analysis/prisma_flow_diagram.py >/tmp/v116_structured_flow.log
...
python3 analysis/source_b_m5_200trial_sensitivity.py --n-bootstrap 1000 >/tmp/v116_m5_200trial.log
...
if pdftotext paper/latex/main.pdf - | grep -E "PRISMA-Informed|Systematic Review:" >/tmp/v116_forbidden_pdf_terms.log; then
  echo "Manuscript PDF still contains PRISMA/systematic-review branding:" >&2
  cat /tmp/v116_forbidden_pdf_terms.log >&2
  exit 1
fi
...
cp docs/RELEASE_V1.16.md "$release_dir/README.md"
...
(cd "$release_dir" && shasum -a 256 -c CHECKSUMS.txt >/tmp/v116_release_checksums.log)
unzip -t "$zip_path" >/tmp/v116_release_zip.log
...
required_files=(
  ...
  "$release_dir/replication/tools/build_v116_release.sh"
  "$release_dir/replication/docs/RELEASE_V1.16.md"
)
