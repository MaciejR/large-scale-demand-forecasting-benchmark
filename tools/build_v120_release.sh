...
release_dir="release/zenodo-v1.20-ijf-submission-package"
zip_path="${release_dir}.zip"
pdf_name="demand-forecasting-ijf-submission-package-v1.20.pdf"
...
python3 analysis/prisma_flow_diagram.py >/tmp/v120_structured_flow.log
...
python3 analysis/source_b_paired_panel_report.py --runs \
  ...
  --n-bootstrap 1000 >/tmp/v120_source_b_report.log
python3 analysis/source_b_m5_200trial_sensitivity.py --n-bootstrap 1000 >/tmp/v120_m5_200trial.log
python3 analysis/source_b_m5_panel_scaled_metric.py >/tmp/v120_m5_panel_scaled.log
python3 analysis/source_b_prespecified_baseline_sensitivity.py >/tmp/v120_prespecified_baseline.log
...
if pdftotext paper/latex/main.pdf - | grep -E "PRISMA-Informed|Systematic Review:|Supplemental Heuristic-Weight Diagnostics|Task-clustered CR2|rma\\.mv" >/tmp/v120_forbidden_pdf_terms.log; then
  echo "Manuscript PDF still contains removed review-risk terms:" >&2
  cat /tmp/v120_forbidden_pdf_terms.log >&2
  exit 1
fi

if ! pdftotext paper/latex/main.pdf - | grep -F "GitHub release v1.20" >/tmp/v120_required_pdf_terms.log; then
  echo "Manuscript PDF does not contain GitHub release v1.20 data-availability text." >&2
  exit 1
fi
...
cp docs/RELEASE_V1.20.md "$release_dir/README.md"
...
(cd "$release_dir" && shasum -a 256 -c CHECKSUMS.txt >/tmp/v120_release_checksums.log)
unzip -t "$zip_path" >/tmp/v120_release_zip.log
...
required_files=(
  ...
  "$release_dir/replication/tools/build_v120_release.sh"
  "$release_dir/replication/docs/RELEASE_V1.20.md"
)
