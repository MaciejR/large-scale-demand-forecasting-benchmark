...
timestamp.  After the DOI metadata files are committed, pass
`--doi-update-commit "$(git rev-parse HEAD)"` to record the full SHA of that DOI
metadata commit in the publication log.
