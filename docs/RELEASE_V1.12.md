...
`tools/publish_v112_zenodo.sh` and the token-readiness audit read only known
Zenodo token variables from a local ignored `.env` file when the same variables
are not already exported; the Python uploader itself reads only process
