...
add `--require-complete` to fail on remaining placeholders, mismatched DOI URLs,
...
the irreversible publish action requires both an explicit `--publish` flag and
`--deposition-id` for an existing unpublished draft.  Add `--output-json` to
persist the Zenodo response for the publication-log helper.
...
`tools/publish_v112_zenodo.sh`, `tools/zenodo_upload_v112.py`, and the
token-readiness audit read only known Zenodo token variables from a local
ignored `.env` file when the same variables are not already exported.
...
ID, record URL, DOI, current commit, package SHA, and `published_at_utc`
timestamp.  The first log update records the package identity uploaded to
Zenodo.  After the DOI metadata files are committed, pass `--doi-update-commit
"$(git rev-parse HEAD)"` to record the full SHA of that DOI metadata commit
without overwriting the Zenodo-uploaded target commit or asset SHA.
