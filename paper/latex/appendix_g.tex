...
\S\ref{sec:baseline-reliability}.  Version~1.4 treats the
...
differs across datasets, (ii) the v1.4 matched-panel aggregate-\WAPE{}
repair makes M5/Rohlik more conservative than the legacy per-series view,
and (iii) the Favorita \texttt{direct\_scaled} run still has an unresolved
integrity flag at $h=7$.
...
\subsection{v1.4 Matched-Panel Protocol Check}

The v1.4 matched-panel repair recomputes the M5/Rohlik LightGBM protocol
...
than recursive on Rohlik at all horizons.  The horizon-scaled direct run
equals the unscaled direct run at $h=7$ on both datasets, as expected when
both use 300 trees.  This resolves the same-tree-count anomaly for M5 and
Rohlik; the unresolved anomaly is now limited to the legacy Favorita
\texttt{direct\_scaled} file.
...
The Azure cost tables are retained for audit, but v1.4 does not use them
...
\midrule
M5 total & 0.877 & 0.810 & $-0.067$ & mismatch; reconcile from run ledger \\
Rohlik total & 0.110 & 0.120 & $+0.010$ & mismatch; reconcile from run ledger \\
Favorita total & 0.777 & 1.119 & $+0.343$ & mismatch; reconcile from run ledger \\
Favorita direct & 0.687 & 1.030 & $+0.343$ & mismatch; likely source of total gap \\
M5 Seasonal Naive & 0.059 & 0.146 & $+0.087$ & mismatch \\
M5 direct & 0.724 & 0.571 & $-0.153$ & mismatch \\
\bottomrule
\end{tabularx}
\end{table}

The cost plots in \S\ref{sec:cost-accuracy} therefore remain cost-error
scatterplots.  The v1.4 matched-panel repair writes a local run ledger for
the M5/Rohlik rerun; the older Azure summary/detail tables still need a
single reconciled ledger before cost claims are promoted beyond descriptive
accounting.
...
environment.  The v1.4 matched-panel runs store shared series identifiers,
forecast origins, actuals, per-model predictions, and paired bootstrap
contrasts for M5/Rohlik.  The same requirement applies to Favorita,
TimesFM~2.5, TiRex, and future GPU/batched reruns before Source~B can
enter a broader formal paired analysis.
