...
\texttt{benchmark/results/local\_fm\_sweep.csv}, 700+ data rows)
...
B     & Benchmark papers (incl.\ fev-bench, GIFT-Eval) & 507 \\
...
LOCAL & Our Source~B cells (4~FMs + 3~baselines, MacBook/Azure) & 63 \\
\midrule
\textbf{Total} & & \textbf{${\sim}$720} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[nosep]
  \item \textbf{Source~A} (literature extraction): categories A--F
    (${\sim}$607 rows from 38+ studies, including per-task fev-bench
  ...
  \item \textbf{Source~B} (gap-filling experiments): OWN + LOCAL
    (${\sim}$81 rows). These rows are from our own experiments on
