...
standard error of mean \WAPE{} for each of the 18 Source~B foundation
model cells (Chronos-Bolt-Tiny and TiRex $\times$ 3 datasets $\times$
3 horizons). Each cell samples 100 series (98--100 after \WAPE{}
filtering). The bootstrap validates that $n = 100$ produces stable
\WAPE{} estimates for the meta-regression of \S6.

\begin{table}[htbp]
\centering
\caption{Bootstrap standard error and 95\% CI for Source~B FM cells.
Two rows per (dataset, horizon) cell correspond to Chronos-Bolt-Tiny
and TiRex respectively.}\label{tab:bootstrap-se}
\footnotesize
\begin{tabular}{llrrrrr}
\toprule
Dataset  & Horizon & $n_\text{valid}$ & \WAPE{} mean & Bootstrap SE & 95\% CI & CI width \\
\midrule
...
$\pm$0.027 \WAPE{} (Rohlik $h = 28$). Interpretation:
...
documented in \S5.4.5 and is the reason we exclude M5 \WAPE{} cells
from the primary meta-regression intercept (\S6.7, excl-M5 model).
