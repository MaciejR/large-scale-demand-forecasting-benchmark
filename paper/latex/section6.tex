...
forecasting.  The pipeline yields $k = 450$ paired comparisons across
8~datasets and 11~foundation model entries.  Appendix~\ref{app:sensitivity}
...
The 450~pairs come from three sources:
\begin{itemize}
...
\paragraph{Full-pool intercept ($k = 450$).}

\begin{quote}
$\hat{\Delta}\;\text{(FM} - \text{baseline)} = -0.150$,\quad
$\text{SE} = 0.045$,\quad
$t(449) = -3.33$,\quad $p = 0.001$,\quad
95\,\% CI $[-0.240,\; -0.060]$.
\end{quote}
...
$Q(449) = 48{,}672$, $p < .0001$.  The enormous $Q$ reflects
...
\WAPE{} / ND         & 174 & $-0.070$ & $[-0.164,\; +0.023]$ & $0.14$    & Directionally favorable, n.s. \\
\bottomrule
\end{tabular}
\end{table}
...
$k = 450$ pool.  The results are robust: the sign is consistently
...
\midrule
Full pool         & 450 & $-0.150$ & $[-0.240,\; -0.060]$ & 0.001 & Primary \\
Excl-M5           & 378 & $-0.116$ & $[-0.194,\; -0.038]$ & 0.004 & M5 not driving result \\
\WAPE{}-only      & 174 & $-0.070$ & $[-0.164,\; +0.023]$ & 0.14 & Directionally favorable, n.s. \\
SQL-only          & 138 & $-0.278$ & $[-0.378,\; -0.178]$ & $< .0001$ & Strongest signal \\
Source B only     &  36 & $-0.229$ & $[-0.569,\; +0.110]$ & 0.180 & Low power ($k = 36$) \\
Source A only     & 414 & $-0.113$ & $[-0.159,\; -0.067]$ & $< .0001$ & Significant \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
  \item \textbf{M5 is not driving the result.}  Excluding M5
    ($k = 378$) barely changes the estimate ($-0.116$ vs $-0.150$)
...
\midrule
Full pool     & 450 & 48{,}672 & 0.0072 & 0.0221 & 0.0003 & Primary \\
Excl-M5       & 378 &  17{,}311 & --- & --- & --- & \\
SQL-only      & 138 &   6{,}303 & --- & --- & --- & Lowest $Q$ \\
\WAPE{}-only  & 174 &  27{,}081 & --- & --- & --- & \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Cost-accuracy Pareto frontier.
  ...
  hardware cost basis.  The four FMs span 9\,M--120\,M parameters;
...
With $k = 450$ pairs across 9~datasets and 14~FMs, the meta-regression
...
\begin{enumerate}
  \item \textbf{Metric} (\texttt{metric\_name}; $F(2, 447) = 6{,}695$,
  ...
  \item \textbf{Dataset} (\texttt{dataset\_norm}; $F(7, 442) = 0.72$,
    $p = 0.652$).  Not significant as a moderator once metric is
  ...
  \item \textbf{Model scale} (\texttt{model\_scale}, $\log_{10}$
    params; $F(1, 448) = 20.7$, $p < .0001$).  Coefficient is
