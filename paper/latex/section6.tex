...
\begin{quote}
$\hat{\Delta}\;\text{(FM} - \text{baseline)} = -0.190$,\quad
$\text{SE} = 0.043$,\quad
$t(542) = -4.41$,\quad $p < .0001$,\quad
95\,\% CI $[-0.275,\; -0.105]$.
\end{quote}
...
$\hat{\sigma}^2_{\text{paper}} = 0.0248$; the crossed model-name
component is
...
$Q(542) = 53{,}038$, $p < .0001$.  The enormous $Q$ reflects
...
\MASE{}              & 166 & $-0.151$ & --- & --- & (intercept from moderator) \\
\WAPE{} / ND         & 211 & $-0.112$ & $[-0.204,\; -0.019]$ & $0.019$   & FM significantly better \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{statistically significant} ($\hat{\Delta} = -0.112$,
$p = 0.019$), but the effect size is roughly one-third of the SQL
finding.
...
\midrule
Full pool         & 543 & $-0.190$ & $[-0.275,\; -0.105]$ & $< .0001$ & Primary \\
Excl-M5           & 468 & $-0.180$ & $[-0.276,\; -0.084]$ & 0.0003 & M5 not driving result \\
\WAPE{}-only      & 211 & $-0.112$ & $[-0.204,\; -0.019]$ & 0.019 & Significant \\
...
\begin{enumerate}
  \item \textbf{M5 is not driving the result.}  Excluding M5
    ($k = 468$) barely changes the estimate ($-0.180$ vs $-0.190$)
    and remains highly significant ($p = 0.0003$).

  \item \textbf{SQL vs \WAPE{} is the dominant moderator.}  The SQL-only
    slice is the strongest finding ($\Dhat = -0.318$, $p < .0001$);
    the WAPE-only slice is also significant ($\Dhat = -0.112$,
    $p = 0.019$) but with a smaller effect.  This metric gradient is
...
\midrule
Full pool     & 543 & 53{,}038 & 0.0072 & 0.0248 & 0.0003 & Primary \\
Excl-M5       & 468 &  21{,}469 & --- & --- & --- & \\
...
($\hat{\sigma}^2_1 = 0.0072$) and paper ($\hat{\sigma}^2_2 = 0.0248$)
...
With $k = 543$ pairs across 9~datasets and 14~FMs, the meta-regression
...
\begin{enumerate}
  \item \textbf{Metric} (\texttt{metric\_name}; $F(2, 540) = 6{,}792$,
  ...
  \item \textbf{Dataset} (\texttt{dataset\_norm}; $F(8, 534) = 1.02$,
    $p = 0.422$).  Not significant as a moderator.  The raw
  ...
  \item \textbf{Model scale} (\texttt{model\_scale}, $\log_{10}$
    params; $F(1, 541) = 0.24$, $p = 0.627$).  Not significant.
    Within Source~B,
