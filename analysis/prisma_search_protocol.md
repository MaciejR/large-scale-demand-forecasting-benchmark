...
## Current Extraction Status and Limitations

1. The retained auditable search log covers 25 query families.
2. The source map includes newer model families added during the repair cycle:
   TiRex, TabPFN-TS, Toto, Sundial, Moirai 2.0, Chronos-2, and TimesFM 2.5.
3. The frozen quantitative extraction is `analysis/extraction_schema.csv`
   with 802 rows.  These are result rows, not independent studies.
4. GIFT-Eval and fev-bench retail rows are expanded at task/model/metric level
   because the current reanalysis is a cross-benchmark result-row synthesis.
5. The PRISMA flow diagram is generated by `analysis/prisma_flow_diagram.py`
   and written to `analysis/figures/prisma_flow.pdf` and `.png`.
6. The review remains PRISMA-informed, not PRISMA-compliant: Scopus and Web of
   Science exports were not retained, screening was not duplicated by an
   independent reviewer, and forward/backward citation chasing was not
   exhaustively logged.
7. The study-characteristics and risk/provenance tables generated by
   `analysis/meta_regression.R` cover analysis-contributing sources and
   benchmark suites.  They are not a duplicate-screened risk-of-bias table for
   every candidate record in the 49-record source map.
