...
- Legacy Source B contributes a separate descriptive 45-row contrast file; the
  v1.4 matched-panel Source B repair is generated by
...
one common panel of series and forecast origins. Version 1.4 repairs M5/Rohlik
with a matched 100-series panel. For the repair rerun, use the matched-panel
runner rather than the older MLflow-only gap-filling runner.
...
python benchmark/code/experiments/run_source_b_paired_panel.py \
  ...
  --horizons 7 14 28 \
  --max-series 100 \
  --run-id source_b_repair_baselines_m5_rohlik_100
```
...
$HOME/venvs/fm-local/bin/python benchmark/code/experiments/run_source_b_paired_panel.py \
  ...
  --horizons 7 14 28 \
  --max-series 100 \
  --run-id source_b_repair_fm_chronos_bolt_m5_rohlik_100
```
